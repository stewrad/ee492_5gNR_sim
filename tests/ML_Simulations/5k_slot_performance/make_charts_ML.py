...
def is_ml_series(gval: object) -> bool:
    """
    Returns True if the group label appears to be the ML method/line.

    This will match labels such as:
    - ML
    - ml
    - ML Controller
    - Logistic ML
...
def plot_chart(df: pd.DataFrame, spec: ChartSpec, out_dir: str) -> str:
    ...
    # Keeps gridlines behind plotted data
    ...
    if spec.groupby:
        ...
        # Plot non-ML groups first and ML groups last.
        # Anything drawn later with a higher zorder will appear visually on top.
        groups = list(grouped.groupby(spec.groupby))
        groups.sort(key=lambda item: is_ml_series(item[0]))

        for gval, gdf in groups:
            gdf = gdf.sort_values(spec.x)

            ml = is_ml_series(gval)

            zorder = 10 if ml else 2
            linewidth = 2.8 if ml else 1.6
            markersize = 6 if ml else 4
            scatter_size = 50 if ml else 30
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate charts from Excel using charts.yml."
    )
