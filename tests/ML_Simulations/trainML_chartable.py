...
def mean_if_exists(dataframe, col):
    if col in dataframe.columns:
        return dataframe[col].mean(numeric_only=True)
    return pd.NA


def first_if_exists(dataframe, col):
    if col in dataframe.columns and not dataframe[col].dropna().empty:
        return dataframe[col].dropna().iloc[0]
    return pd.NA
...
# Keep these names matched to your existing charting YAML / parser columns
chart_metric_cols = [
...
for _, row in unique_conditions.iterrows():
    ...
    out_row = {
        ...
        # Keep these so your existing charts.yml filters still work
        "rvSeq": first_if_exists(subset, "rvSeq"),
        "DelayProfile": first_if_exists(subset, "DelayProfile"),
        "nTxAnts": first_if_exists(subset, "nTxAnts"),
        "nRxAnts": first_if_exists(subset, "nRxAnts"),
        "NumLayers": first_if_exists(subset, "NumLayers"),
    ...
    # Copy averaged simulation metrics using the SAME names as runlog_results.xlsx
    for col in chart_metric_cols:
        out_row[col] = mean_if_exists(subset, col)
...
# =========================
# 9. Pick recommended modulation per SNR/HARQ condition
# =========================
recommendation_rows = []
...
for (snr, harq), group in decision_df.groupby(["SNRdB", "NHARQProcesses"]):
    ...
    if not safe_group.empty:
        # Choose best throughput among low-risk candidates
        best_row = safe_group.sort_values(
            ["Average Throughput (Mbps)", "Throughput Efficiency (%)"],
            ascending=False
        ).iloc[0]
        reason = "Highest throughput below risk threshold"
    else:
        # If all are risky, choose the least risky candidate
        best_row = group.sort_values(
            ["Predicted Risk Probability", "Estimated Latency (ms)"],
            ascending=True
        ).iloc[0]
    ...
    recommendation_rows.append({
        "SNRdB": snr,
        "NHARQProcesses": harq,
        "Recommended Modulation": best_row["Modulation"],
...
# Add recommended modulation back into the chartable decision table
...
decision_df["Is Recommended"] = (
    decision_df["Modulation"] == decision_df["Recommended Modulation"]
).astype(int)
...
metric_cols = [c for c in metric_cols if c in ml_selected_df.columns]
...
with pd.ExcelWriter("ml_results_output_extended.xlsx", engine="openpyxl") as writer:
    # First sheet is chart-friendly by default
