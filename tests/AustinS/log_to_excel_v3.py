...
v3 additions:
- preserves compatibility with legacy/original NR_sim_parallel.m logs
- adds ML-only fields emitted by NR_sim_parallel_ML_logging_aligned.m

Requires:
  pip install pandas openpyxl

Usage examples:
  python3 -m log_to_excel_v3 -d logs/ -ex .
  python3 -m log_to_excel_v3 -d logs/ -ex results.xlsx --dedupe
"""
...
def parse_runlog(text: str, source_file: str) -> Dict[str, Any]:
    ...
    out["Modulation"] = (
        ...
        or _re_str(r"Modulation:\s*([A-Za-z0-9_]+)", text)
    )
    ...
    # --- ML-only header fields ---
    out["ControlMode"] = (
        _re_str(r"ControlMode:\s*\"([^\"]+)\"", text)
        or _re_str(r"ControlMode:\s*([A-Za-z0-9_\-]+)", text)
    )
    out["FixedMod"] = (
        _re_str(r"FixedMod:\s*\"([^\"]+)\"", text)
        or _re_str(r"FixedMod:\s*([A-Za-z0-9_\-]+)", text)
    )
    out["MLRiskThr"] = _re_float(r"MLRiskThr:\s*([\-0-9]*\.?[0-9]+)", text)
    out["DatasetMode"] = _re_int(r"DatasetMode:\s*([0-9]+)", text)

    # --- Results section ---
    out["HARQ Type"] = _re_str(r"HARQ Type:\s*(.+)", text)

    # Summary-level ML-only fields may appear later and are useful even if header exists
    out["Fixed Modulation"] = _re_str(r"Fixed Modulation:\s*(.+)", text)
    out["16QAM TB Count"] = _re_int(r"16QAM TB Count:\s*([0-9]+)", text)
    out["64QAM TB Count"] = _re_int(r"64QAM TB Count:\s*([0-9]+)", text)
    out["Average Predicted Risk"] = _re_float(r"Average Predicted Risk:\s*([\-0-9]*\.?[0-9]+)", text)
    out["Average Recent NACK Rate"] = _re_float(r"Average Recent NACK Rate:\s*([\-0-9]*\.?[0-9]+)", text)
    out["Dataset File"] = _re_str(r"Dataset File:\s*(.+)", text)
    ...
    out["Effective Code Rate"] = (
        ...
        or _re_float(r"Effective Code Rate \(InfoBits / CodedBits\):\s*([0-9]*\.?[0-9]+)", text)
    )
    ...
    # Prefer summary-level fixed modulation if present; otherwise fall back to header FixedMod
    if not out.get("Fixed Modulation") and out.get("FixedMod"):
        out["Fixed Modulation"] = out["FixedMod"]
...
def main():
    ...
    all_cols = TRACE_COLUMNS + BASE_COLUMNS + ML_COLUMNS + layer_sinr_cols + layer_evm_cols
