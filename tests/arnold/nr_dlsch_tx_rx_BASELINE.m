...
function nr_dlsch_tx_rx_BASELINE(SNRdB, NHARQProcesses)
    % SET TEST PARAMETERS HERE: 
    % ------------------------------------------------------------------------------------------------                                                                                        % [0 2 4 6 8 10 12] dB // baseline 8.0dB; 
    Modulation = '256QAM';                                                                             % must be 'QPSK', '16QAM', '64QAM', '256QAM', '1024QAM'; // baseline QPSK                                                                               % [1 2 4 8 16]; // baseline is 16
    ...
    nTxAnts = 2;                                                                                       % [1 2 4 8] // baseline is Tx = 8 
    nRxAnts = 2;                                                                                       % [1 2 4 8] // baseline is Rx = 8
    NumLayers = min(2, min(nTxAnts, nRxAnts)); %%%%%%%%%%%%%                                                                                     % Set to 2 for all but Tx/Rx Ant = 1;
    ...
    if logging == 1
        ...
        figDir = fullfile(pwd, 'test\arnold\logs'); 
        if ~exist(figDir,'dir')
            mkdir(figDir);
        end
        ...
        rvStr = rvStr(1:end-1); % remove trailing underscore %%%%%%%%%%%%%%%
    ...
    % HARQ delivery tracking (CRITICAL)
    tbDelivered = false(pdsch.NumCodewords, NHARQProcesses);
    tbRetransmitted = false(pdsch.NumCodewords, NHARQProcesses);
    
    % For tracking per-transmission statistics
    perSlotSuccess = zeros(totalNoSlots, pdsch.NumCodewords);
    perSlotBLER = zeros(totalNoSlots, 1); %%%%%%%%%%%%%%%%%% Changed from perslot BER to BLER
    ...
    for nSlot = 0:totalNoSlots-1
        ...
        for layerIdx = 1:pdsch.NumLayers
            ...
            symTx = pdschSymbols(:, layerIdx);      % TRUE transmitted symbols
            symRx = layerSymbols;               % equalized received symbols
    
            error = symRx - symTx;
    
            signalPower = mean(abs(symTx).^2);
            noisePower  = mean(abs(error).^2);
        ...
        for cwIdx = 1:pdsch.NumCodewords
            ...
            harqId = harqEntity.HARQProcessID + 1; % if needed: (cwIdx)
            isNew = harqEntity.NewData(cwIdx); %%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            if isNew
                ...
                % Reset delivery flag for new TB
                tbDelivered(cwIdx, harqId) = false;
            else
                totalRetransmissions = totalRetransmissions + 1;
                tbRetransmitted(cwIdx, harqId) = true;
            end
        
            % --- Correct HARQ success tracking ---
            if ~blkerr(cwIdx) && ~tbDelivered(cwIdx, harqId)
                successfulBlocks = successfulBlocks + 1;
                totalRxBits = totalRxBits + trBlkSizes(cwIdx);
        
                tbDelivered(cwIdx, harqId) = true;
                perSlotSuccess(nSlot+1, cwIdx) = 1;
        
            elseif harqEntity.SequenceTimeout(cwIdx)
                blockErrors = blockErrors + 1;
            end
        end
        totalTransmissions = totalInitialTransmissions + totalRetransmissions;
        disp("Updating channel estimate...");
    end
    
    numRetransmittedTBs = sum(tbRetransmitted(:));
    retransmissionRate = numRetransmittedTBs / max(1,totalInitialTransmissions) * 100;
    
    for harqId = 1:NHARQProcesses
        for cwIdx = 1:pdsch.NumCodewords
            if ~tbDelivered(cwIdx, harqId)
                blockErrors = blockErrors + 1;
            end
        end
    end
    ...
    % --- BLER metrics ---
    finalBLER = (totalInitialTransmissions - successfulBlocks) / max(1, totalInitialTransmissions); % post-HARQ (what you have now)
    ...
    probRetx = numRetransmittedTBs / max(1,totalInitialTransmissions) * 100;
    avgRetx  = totalRetransmissions / max(1,totalInitialTransmissions) * 100;
    
    fprintf('Prob Retx (correct): %.2f%%\n', probRetx);
    fprintf('Avg Retx  (old): %.2f%%\n', avgRetx);
end